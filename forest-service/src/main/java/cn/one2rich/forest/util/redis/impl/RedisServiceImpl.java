package cn.one2rich.forest.util.redis.impl;

import cn.one2rich.forest.util.redis.RedisResult;
import cn.one2rich.forest.util.redis.RedisService;
import cn.one2rich.forest.config.RedisProperties;
...
public class RedisServiceImpl implements RedisService {
        ...
        if (pool == null) {
          pool =
              new JedisPool(
                  ...
                  redisProperties.getSslSocketFactory(),
                  redisProperties.getSslParameters(),
                  redisProperties.getHostnameVerifier());
        }
      }
    }
    return pool;
  ...
  public <T> RedisResult<T> getListResult(String key, Class<T> elementClazz) {
    ...
    try {
      ...
      logger.error(
          "Can not unserialize list to [{}] with string [{}]", elementClazz.getName(), value);
      // 到此步直接视为无值
      redisResult.setExist(false);
    }
  ...
  private static <T> JavaType getCollectionType(
      Class<? extends Collection> collectionClazz, Class<T> elementClazz) {
   ...
   * @param key
   * @param value
   * @param nxxx 缓存写入值模式 详见 {@link RedisService#NXXX_SET_IF_EXISTS}, {@link
   *     RedisService#NXXX_SET_IF_NOT_EXISTS}
